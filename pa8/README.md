[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/_Ge4Y2U_)

# Programming Assignment 8: microC Step 2 Submission Updates

## Development Implementation Notes:
* **Parser Validation (`MicroC.g4`)**: Successfully assigned proper AST classes (`BinaryOpNode`, `UnaryOpNode`) inside inline Python ANTLR actions.
* **Visitor Code Generator (`CodeGenerator.py`)**: Traversed the ASTs manually. 
    * Fully bound operations for floats / integral operations generating `FAdd`, `FSub`, `Add`, etc. 
    * Dynamically mapped literal loads to corresponding destination registers via instruction definitions (`Li`, `Fimm`).
    * Configured string loading sequence via memory lookup (`La` + `PutS`), eliminating manual type cast issues for constants.
* **Testing:** Ran automated parser and assembly verifications locally on `test0-test10.uC`. Output `.asm` sequences match correct structure definitions.
