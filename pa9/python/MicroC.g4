...
if_stmt returns [IfStatementNode node]: 'if' '(' cond ')' '{' statements '}' {$node = IfStatementNode($cond.node, $statements.node, None);}
      | 'if' '(' cond ')' '{' ts=statements '}' 'else' '{' es=statements '}' {$node = IfStatementNode($cond.node, $ts.node, $es.node);};

while_stmt returns [WhileNode node] : 'while' '(' cond ')' '{' statements '}' {$node = WhileNode($cond.node, $statements.node);} ;
...
cond returns [CondNode node] : e1=expr cmpop e2=expr {$node = CondNode($e1.node, $e2.node, $cmpop.text);} ;
