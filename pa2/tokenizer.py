...
class Tokenizer:
    ...
    def nexttoken(self) -> Token:
        ...
        match char:
            ...
            case 'i': # Integer Declaration: i + variable
                # Skip whitespace
                while self.cs.peek() in {' ', '\n', '\r', '\t'}:
                    self.cs.advance()
                    
                var = self.cs.read()
                if var not in VALID_VARS:
                     # Must match 'Invalid variable character: {var}'
                     raise ValueError(f"Invalid variable character: {var!r}")
                return Token(TokenType.INTDEC, lexeme=f"i{var}", name=var)

            case 'p': # Print: p + variable
                # Skip whitespace
                while self.cs.peek() in {' ', '\n', '\r', '\t'}:
                    self.cs.advance()
                ...
                if var not in VALID_VARS:
                     raise ValueError(f"Invalid variable character: {var!r}")
                return Token(TokenType.PRINT, lexeme=f"p{var}", name=var)
    ...
    def readintliteral(self, firstchar: str) -> tuple[str, int]:
        ...
        if firstchar == '0':
            if not self.cs.eof() and self.cs.peek().isdigit():
