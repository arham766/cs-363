...
def parse(ts: TokenStream) -> ASTNode:
    ...
    if t.tokentype == TokenType.FLOATDEC:
        ts.read()
        v = expect(ts, TokenType.VARREF)
        node = FloatDclNode(v.lexeme)
        expect(ts, TokenType.EOF)
        return node

    if t.tokentype == TokenType.STRDEC:
        ts.read()
        v = expect(ts, TokenType.VARREF)
        node = StrDclNode(v.lexeme)
        expect(ts, TokenType.EOF)
        return node
    ...
    raise ParseError(
        f"Expected TokenType.PRINT, TokenType.INTDCL/INTDEC, FLOATDEC, STRDEC, or TokenType.VARREF; got {t.tokentype}"
    )
...
def parse_expression(ts: TokenStream) -> ASTNode:
    ...
    while ts.peek().tokentype != TokenType.EOF:
        ...
        if tok.tokentype == TokenType.STRLIT:
            if not expect_operand:
                 raise ParseError("Expected operator or rparen after string literal")
            tok = ts.read()
            if tok.strvalue is None:
                raise ParseError("Malformed STRLIT token")
            valstack.append(StrLitNode(tok.strvalue))
            expect_operand = False
            continue
